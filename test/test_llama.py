...
def parse_args():
    ...
    parser.add_argument(
        "--devices", type=int, default=2, help="Number of devices/stages (default: 2)"
    )
    parser.add_argument(
        "--dp_degree",
        type=int,
...
def main(args: argparse.Namespace):
    ...
    compiled = piper_setup(
        model, [x], backend=piper, num_stages=num_stages, num_devices=num_devices
    )
    ...
    if args.tracing:
        ...
        ray.timeline(
            f"out/{args.model}-pp{num_devices//args.dp_degree}-dp{args.dp_degree}-{args.schedule}.json"
        )
...
if __name__ == "__main__":
    ...
    piper_coordinator = PiperProgramCoordinator.remote(
        dp_degree=args.dp_degree, world_size=args.devices
    )
