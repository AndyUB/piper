...
class ConditionalFeedForward(nn.Module):
    ...
    def forward(self, x: Tensor, dispatch_mask: Tensor, combine_weights: Tensor, capacity: int) -> Tensor:
        ...
        """
        # all gather
        expert_inputs = torch.einsum("tec,td->ecd", dispatch_mask.type_as(x[0]), x) # [E, C, D]
        # reduce scatter
        x1 = F.silu(torch.einsum('ecd, emd -> ecm', expert_inputs, self.w1))
        # reduce scatter
        x3 = torch.einsum('ecd, emd -> ecm', expert_inputs, self.w3)
        expert_output =  torch.einsum('ecm, edm -> ecd', (x1 * x3), self.w2)
        # all gather
        expert_output = expert_output.redistribute(self.device_mesh, [Replicate()])
