...
def validate_schedule(
    schedule: list[list[Task | None]], dag_edges: list[DAGEdge], num_mbs: int
) -> None:
    """
    ...
    for stage_id in range(num_devices):
        for time_step in range(num_steps):
            ...
            if task is not None:
                ...
                microbatch_tasks[task.mb_idx].add(
                    (task.stage_id, task.is_fwd, task.upd)
                )
    ...
    for mb_idx, tasks in microbatch_tasks.items():
        ...
        bwd_stages = {
            stage_id for stage_id, is_fwd, upd in tasks if not is_fwd and not upd
        }
...
def piper_exec(model, schedule, inputs, truth, loss_fn, num_mbs, num_stages):
    ...
    events = dict(
        [
            (mb_idx, [threading.Event() for _ in range(num_stages)])
            for mb_idx in range(num_mbs)
        ]
    )
