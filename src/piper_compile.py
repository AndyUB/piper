...
from .piper_exec import CompType
...
def order_p2p_comms(schedule, num_devices, num_stages, stage_to_device):
    is_fwd_bwd = lambda task: task is not None and task.type != CompType.UPD
    schedule = [
    ...
    while True:
        ...
        for rank, time_step in rank_to_next_task.items():
            if time_step < len(schedule[rank]):
                ...
                dependee = None
                if task.type == CompType.FWD and task.stage_id > 0:
                    dependee = (task.stage_id - 1, task.stage_id, task.mb_idx)
                elif task.type == CompType.BWD and task.stage_id < num_stages - 1:
                    dependee = (task.stage_id + 1, task.stage_id, task.mb_idx)
        ...
        for rank, task in executable_tasks:
            if task.type == CompType.FWD and task.stage_id < num_stages - 1:
                p2p_comms.append((task.stage_id, task.stage_id + 1, task.mb_idx))
            elif task.type == CompType.BWD and task.stage_id > 0:
                p2p_comms.append((task.stage_id, task.stage_id - 1, task.mb_idx))
