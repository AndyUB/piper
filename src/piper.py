...
@register_backend
def piper(gm: torch.fx.GraphModule, example_inputs: list[torch.Tensor], **kwargs):
    ...
    serializable_examples: list[int | torch.Tensor] = []
    input_idxs: list[int] = []
    ...
    actor: ActorProxy[PiperActor] = piper_metadata.actors[actor_id]
    dp_rank = int(os.environ["PIPER_DP_RANK"])
    dp_degree = int(os.environ["PIPER_DP_DEGREE"])
    global_rank = dp_rank * dp_degree + actor_id
    ray.get(
