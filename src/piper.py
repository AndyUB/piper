...
from .piper_utils import RemoteTensor, serialize_graphmodule, piper_tls
import threading
...
def distributed_stage(stage_id, actor_id=None, mb=None, optim=None):
    ...
    if actor_id is None:
        actor_id = stage_id
    piper_tls.current_mb = mb
    if actor_id not in piper_tls.actors:
        actor = PiperActor.remote(actor_id, optim_fn=optim)
        piper_tls.actors[actor_id] = actor
    piper_tls.current_stage = stage_id
    piper_tls.current_actor = actor_id
    piper_tls.first_graph_of_stage = True
...
def piper(gm, example_inputs, **kwargs):
    ...
    stage_id = piper_tls.current_stage
    actor_id = piper_tls.current_actor
    actor = piper_tls.actors[actor_id]
    ray.get(
    ...
    first_graph_of_stage = piper_tls.first_graph_of_stage
    if first_graph_of_stage:
        piper_tls.first_graph_of_stage = False
    ...
    def run_remote_subgraph(*args):
        ...
        mb_idx = piper_tls.current_mb
        if first_graph_of_stage:
            piper_tls.events[mb_idx][stage_id].wait()
        ...
        for arg in args:
            if isinstance(arg, RemoteTensor):
                piper_tls.dag.add((arg.get_stage_id(), stage_id))
        ...
        if first_graph_of_stage:
            piper_tls.events[mb_idx][stage_id].clear()
