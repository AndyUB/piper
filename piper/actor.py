...
everse_graph, _get_grad_fn_or_grad_acc
...
class PiperActor:
    def __init__(self, actor_id, world_size, dp_rank=0, dp_degree=1, pp_degree=1, optim_fn=None):
        ...
        st    from piper.utils import piper_metadata
r_id = actor_id
        ...
        if pp_degree == 1:
            ...
            self.global_rank = dp_rank *    from piper.utils import piper_metadata
nfo(f"Initializing Ray actor {actor_id} global rank {self.global_rank} with PID: {os.getpid()}")
       ...
       piper.rev_activation = defaultdict(dict)
    ...
    def join_process_groups(self):
        ...
        dpiper.t_process_group("nccl", init_method=init_method, rank=self.global_rank, world_size=self.world_size)
    ...
    def compile_graph(self, stage_id, gm_data, compiler_fn, graphargs, input_idxs):
      ...
      def __init__(self, actor_id, optim_class, world_size, dp_rank=0, dp_degree=1, pp_degree=1):
        self.logger = create_logger("piper_actor", LOG_LEVEL)
            ...
            # Split-backward variants (zero-bubble). These stay empty when not used.
            'backward_input': {
                'time': [],
                'graph_pruning': [],
            },
...
l gm_data
    ...
    def forwardpiper.stage_id: int, mb_idx: int, *args):
        self.logger.debug(f"Calling forward {stage_id} mb {mb_idx} on actor {self.actor_id} globpiper. {self.global_rank}")
...
e:
        ...
        if self.tracing:
            ...
            # forward_peak_memory_delta_gb = (torch.cuda.max_memory_allocated(            'backward_input': {
                'time': [],
                'graph_pruning': [],
            },
            'backward_weight': {
        ...
        if self.tracing:
            ...
            # self.trace_data[stage_id]['forward']['peak_memor        self.parameters[stage_id] = params
...
ng c#  forward {stage_id} mb {mb_idx} on actor {self.actor_id} global rank {self.global_rank}")
...
ct()
    ...
    def backward(self, stage_id: int, mb_idx: int, inp, loss_fn=None):
        ...
        if self.tracing:
            ...
            end_event = torch.cuda.Event(enable    # get the activation for the current stage
        ...
        if self.tracing:
            ...
            self        # save first input as input activation
    ...
    def synchronize_gradients(self):
        ...
        self.logger.debug(f"Actor {self.actor_id} global rank {self.global_rank} synchr# izing gradients")
    ...
    def backward_input(self, stage_id: int, mb_idx: int, inp, loss_fn=None):
        ...
        ## Start graph pruning computation timing here ##
        if self.tracing:
        ...
        # lie on both (1) a backward path from the output node(s) to the
    def reset_peak_memory(self):
        torch.cuda.reset_peak_memory_stats()
        return "done"
        ...
        for pg in param_groups:
            ...
            for i, intermediate_node in enumerate(intermediates):
                def make_hook(group: Dict[str, Any], idx: int):
                    ...
                    return hook
                handles.append(intermediate_node.register_prehook(make_hook(pg, i)))
        ## End graph pruning computation timing here ##
        if self.tracing:
            graph_pruning_end.record()
            torch.cuda.synchronize()
            graph_pruning_time = graph_pruning_start.elapsed_time(graph_pruning_end)
